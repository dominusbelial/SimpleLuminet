#!/usr/bin/env python3
"""
SimpleLuminet V3.3 — Dot Matrix (Fixed Doppler, Gradient Asymmetry)
Changes over V3.2:
  - Doppler factor reduced from 6.0 to 2.0 (smooth gradient, no hard clipping)
  - Bias -0.025 for ~300K dot target (was -0.035 → 494K)
  - Gamma 0.55 (was 0.6)
  - Natural asymmetry through physics, not forced dimming
"""
...
# 90 rotation: bright side on LEFT (matching Luminet)
xv = -yv1; yv = xv1; zv = zv1
vzv = vzv1

# Doppler: smooth gradient, factor 2.0
# Negative vzv = approaching = brighter; Positive vzv = receding = dimmer
# factor 2.0 gives: approaching: 1.0+2.0*|ratio| → up to 2.9, receding: 1.0-2.0*ratio → down to -0.9
# Then clipped to 0-1 with noise, creating natural gradient
doppler = 1.0 - 2.0 * np.clip(vzv / v_kep, -1, 1)
brightness = np.clip(doppler + np.random.normal(0, 0.12, NUM), 0.03, 1.0)
...
# Global normalization (preserves natural asymmetry)
dg = np.clip(dg / dg.max(), 0, 1)

midc = GC//2
left_sum = dg[:, :midc].sum()
right_sum = dg[:, midc:].sum()
print(f"Raw density: left={left_sum:.0f} right={right_sum:.0f} ratio={left_sum/max(right_sum,1):.2f}")
...
left_sum = dg[:, :midc].sum()
right_sum = dg[:, midc:].sum()
print(f"Gamma density: left={left_sum:.0f} right={right_sum:.0f} ratio={left_sum/max(right_sum,1):.2f}")
...
out = f"luminet_v3_3_{ts}.png"
fig.savefig(out, dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
fig.savefig("luminet_v3_3_final.png", dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
plt.close(fig)
print(f"Saved: {out} | luminet_v3_3_final.png")
