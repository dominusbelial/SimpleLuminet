#!/usr/bin/env python3
"""
SimpleLuminet V3.1 — Dot Matrix Printer Emulation
Matches Luminet's original 1979 orientation: bright side on LEFT
Proper LOS Doppler, sharper photon ring
"""
...
v_kep = 1/np.sqrt(r)
vx_disk = -v_kep * np.sin(t)
vy_disk = v_kep * np.cos(t)
...
brightness[ring_mask] = np.clip(brightness[ring_mask] * 1.5, 0, 1.0)
...
dg = gaussian_filter(dg, sigma=1.0)
...
out = f"luminet_v3_1_{ts}.png"
fig.savefig(out, dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
fig.savefig("luminet_v3_1_final.png", dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
plt.close(fig)
print(f"Saved: {out} | luminet_v3_1_final.png")
