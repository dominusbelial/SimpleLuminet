#!/usr/bin/env python3
"""
SimpleLuminet V3.6 — Dot Matrix (Final Tuned Version)
Improvements over V3.5:
  - Weaker lensing deflection (0.5x) — reduces dominant upward halo
  - Stronger receding dim: 0.35 (was 0.55)
  - Sharper sigma: 0.8 (was 1.2)
  - Gamma 0.6 (more contrast)
  - Bias -0.013 for ~280K target
  - DR 0.8 small dots, NUM 500K, spatial Doppler
"""
...
# GRAVITATIONAL LENSING — weaker to reduce upward halo dominance
ib = zv < 0; impact = np.sqrt(xv**2 + yv**2)
photon_sphere = 1.5*BH
deflection_behind = 0.9*BH**1.5 / (np.abs(impact[ib] - photon_sphere) + 0.2*BH)  # reduced from 1.8
deflection_behind = np.clip(deflection_behind, 0, BH*1.0)  # halved max

angle_behind = np.arctan2(yv[ib], np.abs(xv[ib]) + 0.01)
yv[ib] += deflection_behind * np.cos(angle_behind)  # weaker upward deflection

rp_behind = np.sqrt(xv[ib]**2 + yv[ib]**2) + 0.01
xv[ib] += deflection_behind*0.15 * (xv[ib]/rp_behind)  # halved radial push
yv[ib] += deflection_behind*0.15 * (yv[ib]/rp_behind)
...
# Spatial Doppler boost: xv < 0 = left = approaching = brighter
xn = xv[vis]/(RO*0.9)
boost = 1.0 + 10*np.clip(-xn, 0, 1)
...
dg = gaussian_filter(dg, sigma=0.8)  # sharper
...
# Dim receding (right) side — stronger than V3.5
dg_half[:, midc:] *= 0.35  # was 0.55

# Gamma
dg_half = dg_half ** 0.6  # was 0.55
...
BIAS = -0.013  # tuned for ~280K dots
...
out = f"luminet_v3_6_{ts}.png"
fig.savefig(out, dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
fig.savefig("luminet_v3_6_final.png", dpi=DPI, facecolor=PAPER, edgecolor=PAPER,
            bbox_inches=None, pad_inches=0)
plt.close(fig)
print(f"Saved: {out} | luminet_v3_6_final.png")
